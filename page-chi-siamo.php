...
// Se l'utente ha impostato l'Immagine in Evidenza (Featured Image) da Gutenberg, la usiamo come Hero, altrimenti fallback sui setting del CMS Custom
$hero_bg_dynamic = has_post_thumbnail() ? get_the_post_thumbnail_url( get_the_ID(), 'full' ) : ( !empty($chisiamo_hero['bg_image']) ? $chisiamo_hero['bg_image'] : 'https://www.santagatesinelmondo.it/public/header/fratelli%20emigranti.jpg' );
	...
	<section class="section pt-0">
		<div class="w-full">
			...
			while ( have_posts() ) :
                ...
                <!-- Hero Section Dinamica (Gutenberg compatibile) -->
                <section class="hero-section" style="<?php echo esc_attr( $inline_style ); ?>">
                    <div class="container relative z-10 py-24 flex justify-center pt-32">
                        <div class="tonal-panel mx-auto max-w-4xl text-center bg-white/90 backdrop-blur-md">
                            <!-- Titolo preso direttamente dall'Editor della pagina -->
                            <?php the_title( '<h1 class="text-4xl md:text-6xl font-bold mb-6 text-[var(--color-primary)] font-display">', '</h1>' ); ?>

                            <!-- Usa l'Excerpt come Sottotitolo (se presente), altrimenti fallback al Custom CMS -->
                            <?php if ( has_excerpt() ) : ?>
                                <p class="text-xl text-[var(--color-on-surface-muted)] font-body leading-relaxed max-w-2xl mx-auto">
                                    <?php echo get_the_excerpt(); ?>
                                </p>
                            <?php elseif ( !empty($chisiamo_hero['subtitle']) ) : ?>
                                <p class="text-xl text-[var(--color-on-surface-muted)] font-body leading-relaxed max-w-2xl mx-auto">
                                    <?php echo esc_html( $chisiamo_hero['subtitle'] ); ?>
                                </p>
                            <?php endif; ?>
                        </div>
                    </div>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class( 'container pt-16' ); ?>>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
						<div class="entry-content prose prose-lg max-w-none text-[var(--color-on-surface-muted)] font-body leading-loose">
							<?php
                            // Il contenuto standard stampato dall'Editor
                            the_content();
							?>
						</div>
